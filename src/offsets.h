...
// UPDATE THESE after every CS2 patch — they go stale fast.
// Run cs2-dumper.exe while CS2 is open to get fresh values.
// ============================================================
...
namespace offsets {
    ...
    constexpr uintptr_t dwForceJump          = 0x173BCD0;
}
...
namespace cs2 {
    ...
    constexpr uintptr_t m_boneArray          = 0x80;   // within m_modelState

    // CCSPlayerController
    constexpr uintptr_t m_iszPlayerName      = 0x850;

    // C_CSPlayerPawn weapon
    constexpr uintptr_t m_pClippingWeapon    = 0x12F8;
    constexpr uintptr_t m_iItemDefinitionIndex = 0x1B7A; // on weapon entity

    // Bomb / C4
    constexpr uintptr_t m_bBombPlanted       = 0x0;   // on planted c4 entity
    constexpr uintptr_t m_flC4Blow           = 0x510;
}

// Bone indices for CS2 skeleton
namespace bones {
...
// Weapon name lookup — item definition index → display string
// Extend as needed; covers the full buy menu loadout
