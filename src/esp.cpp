...
constexpr WeaponName kWeapons[] = {
    {1,"Deagle"},{2,"Dualies"},{3,"57"},{4,"Glock"},{5,"Bowie"},{7,"AK-47"},
    {8,"AUG"},{9,"AWP"},{10,"FAMAS"},{11,"Auto Sniper"},{12,"Butterfly"},
    {13,"GALIL"},{14,"M249"},{16,"M4A4"},{17,"MAC-10"},{19,"P90"},{20,"Mag-7"},
    {23,"MP5-SD"},{24,"UMP-45"},{25,"XM"},{26,"PP-Bizon"},{27,"MAG-7"},
    {28,"Negev"},{29,"Sawed-Off"},{30,"Tec 9"},{31,"Zeus"},{32,"P2K"},{33,"MP7"},
    {34,"MP9"},{35,"Nova"},{36,"P250"},{38,"Auto Sniper"},{39,"SG 553"},
    {40,"Scout"},{43,"Flashbang"},{44,"HE Grenade"},{45,"Smoke"},{46,"Molotov"},
    {47,"Decoy"},{48,"Incendiary"},{49,"C4"},{57,"Health Shot"},{60,"M4A1"},
    {61,"Usp"},{63,"CZ"},{64,"R8"},{68,"Knife"},{80,"Knife"},{81,"Knife"},
    {82,"Knife"},{83,"Knife"},
};

const char* weapon_name(int id) {
    for (const WeaponName& w : kWeapons) if (w.id == id) return w.name;
...
uint16_t def_index(const Memory& mem, uintptr_t w) {
    return mem.read<uint16_t>(w + offsets::m_AttributeManager +
                              offsets::m_Item + offsets::m_iItemDefinitionIndex);
}

// A weapon's def index is plausible when it names something we know.
...
Careful — I'm going to stop there and hand you the file rather than keep going inline, because it's long. Let me finish it properly:
...
// ── weapon-services discovery ────────────────────────────────────────────
// *** WHY WEAPONS AND THE BOMB CARRIER BOTH VANISHED ***
// Both read through m_pWeaponServices. When that offset was wrong the code
// returned before setting EITHER field, so weapons and the carrier disappeared
// together -- which is exactly the reported pair.
//
// So the offset is found at runtime: scan the pawn for a pointer whose
// m_hActiveWeapon resolves to an entity with a plausible def index. Validated
// against our own pawn first, then confirmed on an enemy.
...
WsvcProbe find_weapon_services(const Memory& mem, uintptr_t el,
    ...
    auto try_pawn = [&](uintptr_t p, uintptr_t off, bool need_name) -> bool {
        ...
        if (!plausible_def(id)) return false;
        if (need_name) {
            char nm[24] = {};
    ...
    for (uintptr_t off = 0x1000; off <= 0x1500; off += 8) {
        ...
        // Confirm on someone else, so we don't latch onto a one-off.
        for (uintptr_t p : pawns) {
            if (p == local_pawn) continue;
            if (try_pawn(p, off, false)) { res.off = off; res.ok = true; return res; }
        }
    }
    return res;
}
...
void ESP::update_world(const Memory& mem, uintptr_t client_base) {
    ...
    static double slots_at = -1e9, info_at = -1e9;
    static bool layout_locked = false;

    if (now - slots_at > 100.0) {
        slots_at = now;
        struct L { uintptr_t off, stride; };
        static const L kL[] = { {0x10,120},{0x10,112},{0x08,120},{0x08,112} };
        ...
        auto try_layout = [&](const L& l, int chunks) {
            ...
            for (uintptr_t e : all) {
                ...
                if (h.health > 0 && h.health <= 150 &&
                    (h.team == 2 || h.team == 3)) got.push_back(e);
            }
        ...
        pawns = bestn > 0 ? std::move(best) : std::vector<uintptr_t>{};
    }
    ...
    if (!c_.wsvc_ok && now - c_.wsvc_try > 1500.0) {
        ...
        const WsvcProbe r = find_weapon_services(mem, el, c_.chunk_off,
                                                 c_.slot_stride, local_pawn,
                                                 probe);
        if (r.ok) { c_.wsvc = r.off; c_.wsvc_ok = true; }
    }

    // ── bone chain ──────────────────────────────────────────────────────
    if (!c_.bones_ok || c_.bone_fail > 250) {
        if (c_.probe_cd > 0) --c_.probe_cd;
        else {
            struct Seed { uintptr_t n, a; };
            static const Seed kSeeds[] = {
                {0x330,0x240},{0x338,0x240},{0x330,0x210},{0x338,0x210},
            ...
            for (const Seed& s : kSeeds) {
                ...
                if (hits >= (test.size() >= 2 ? 2 : 1)) {
                    ...
                    c_.bones_ok = true; c_.bone_fail = 0;
                    break;
                }
            }
            c_.probe_cd = 500;
            ...
            if (valid_ptr(a)) {
                ...
                if (mem.read_bytes(a, raw, sizeof(raw))) {
                    ...
                    for (int i = 0; i < BONE_COUNT; ++i) {
                        ...
                        s.bones[i] = q; s.bone_ok[i] = true; ++cnt;
                    }
    ...
    if (now - info_at > 300.0) {
        ...
        for (Track& t : fresh) {
            ...
            const uint32_t hw = mem.read<uint32_t>(ws + offsets::m_hActiveWeapon);
    ...
    const uintptr_t pl = mem.read<uintptr_t>(client_base + offsets::dwPlantedC4);
    uintptr_t c4 = 0;
    if (looks_like_entity(mem, pl)) c4 = pl;
    else if (valid_ptr(pl)) {
        const uintptr_t in = mem.read<uintptr_t>(pl);
        if (looks_like_entity(mem, in)) c4 = in;
    }

    if (c4) {
        const uintptr_t node = mem.read<uintptr_t>(c4 + offsets::m_pGameSceneNode);
        if (valid_ptr(node)) {
            static const uintptr_t kCand[] = { offsets::m_vecAbsOrigin, 0xD0, 0xC8 };
            ...
            if (c_.c4_ok) {
                Vec3 v{};
                if (read_pos(c_.c4_off, v) && accept(v)) { bomb = v; bomb_on = true; }
                else c_.c4_ok = false;
            }
            if (!c_.c4_ok) {
                for (uintptr_t off : kCand) {
                    ...
                    if (c_.c4_off == off && c_.c4_last_t > 0.0 &&
                        std::fabs(v.z - c_.c4_last.z) < 0.5f) ++c_.c4_stable;
                    else if (c_.c4_off != off) { c_.c4_off = off; c_.c4_stable = 1; }
                    c_.c4_last = v; c_.c4_last_t = now;
                    if (c_.c4_stable >= 3) { c_.c4_ok = true; bomb = v; bomb_on = true; break; }
                }
            }
        }
    }
...
std::vector<PlayerESP> ESP::project(const Memory& mem, uintptr_t client_base,
    ...
    const double rt = interpolate
        ? now_ms() - (double)interp_delay_ms : now_ms();
    ...
    for (const Track& t : tracks_) {
        ...
        e.health = t.health; e.team = t.team; e.distance = t.distance;
        e.has_bones = s.has_bones; e.has_bomb = t.has_bomb;
...
BombESP ESP::project_bomb(const Memory& mem, uintptr_t client_base,
    ...
    const Matrix4x4 vm = mem.read<Matrix4x4>(client_base + offsets::dwViewMatrix);
    const uintptr_t lp = mem.read<uintptr_t>(client_base + offsets::dwLocalPlayerPawn);
