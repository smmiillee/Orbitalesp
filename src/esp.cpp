...
// The radar's PRIMARY path: definition index, self-tested at startup.
...
void ESP::update_world(const Memory& mem, uintptr_t client_base) {
    ...
    // The radar's method, and the reason the old approach failed: the carrier
    // is found from the C4 entity's OWNER handle, not from the player's active
    // weapon. Once discovered, the offset is cached.
    uintptr_t c4_ent  = 0;
    uintptr_t carrier = 0;
    ...
    if (!pawns.empty() && now - c4_at > 500.0) {
        ...
        // Full scan when the cache is cold. Only attempted once the def-index
        // chain has proven itself, so it cannot chase a stale offset.
    ...
    for (uintptr_t p : pawns) {
        ...
        // Reuse the existing track so history, name and weapon survive.
        Track* dst = nullptr;
        for (Track& t : tracks_) {
